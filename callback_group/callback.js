  ...
  function getTeamMembers(index,callback){
    ...
    if(index==0){
        ...
        let  cell=document.createElement("td");
          ...
          //set the timer to 1sec before the display of the first member 
        setTimeout(()=>{
         if( index<team.length){
             let output= `<h3>${team[index].fname} ${team[index].lname}</h3><br>`;
             callback(output);
             //set the timer to 1sec between each did play
            setTimeout(()=>getTeamMembers(index+1,callback),1000)}
          },1000 )
     }
  //calling the function with the index=0 to start from the first member of the array
  //calling the function with the display call back function
